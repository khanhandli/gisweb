<?php 
    ...
    if(isset($_GET['ten_vung'])){
        ...
        $query = "select *,st_x(ST_Centroid(geom)) as x,st_y(ST_Centroid(geom)) as y from public.camhoangdc_1 where LOWER(txtmemo) like '%$name%'";
        $result = pg_query($conn, $query);
        $tong_so_ket_qua = pg_num_rows($result);

        if($tong_so_ket_qua > 0) {
            while($dong = pg_fetch_array($result, null, PGSQL_ASSOC)) {
                $link = "<a href='javascript:void(0);' onclick='di_den_diem(".$dong['x'].",".$dong['y'].")'>here</a>";
